...
def parse_arguments():
    ...
    # Add the URL argument for videos/playlists or a txt file (with URLs)
    parser.add_argument('url', metavar='url', type=str,
                        help='video/playlist url or text file with urls')

    # Set version text
    version_text = '%(prog)s {version}'.format(version=__version__)

    parser.add_argument('-v', '--version', action='version',
                        version=version_text, help='show program version')
...
def on_download_progress(stream, chunk, bytes_remaining):
    ...
    # Print progress bar
    print(Fore.LIGHTGREEN_EX + '[%s] %.2f%%' % (bar, percentage), end='\r')
